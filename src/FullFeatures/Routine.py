...
#----USER1------------------------------------------------------------------
    ...
    for k in range(0,N_idata,n_idata):
        ...
        feat = sigFeatures(is_rut)
        ifourier=tFourier(segment,is_rut,1)
        feat1= lSpeed(segment)
        feat2= sigAltimeter(segment)
        #GYRO
        i_feat= gFeatures(ig_rut)
        i_fourier=tFourier(segment,ig_rut,1)
        user1=np.zeros([1,25])
        user1[0,:] = np.concatenate((feat,ifourier,feat1,feat2,i_feat, i_fourier), axis=None)
        #CLASIFICACION
        #-------------------------------------------------------------------------------------------------------------------------------
        #-----------SIN-PCA--------------------------------------------------------------------------------------------------------------------
        ...
        if k==0:
            ...
            segment_m = mrut[k-150*counter:k-150*counter+n_mdata]
        counter=counter+1
        if len(segment_m) > 0:
            #DESCRIPTORES
            m_rut,mg_rut = magSignal(segment_m,0)
            m_rut=list(filter(None,m_rut))
            mfeat = sigFeatures(m_rut)
            mfourier=tFourier(segment_m,m_rut,0)
            #GYRO
            m_feat= gFeatures(mg_rut)
            m_fourier=gFourier(segment_m,mg_rut,0)
            #USUARIO
            mUser1=np.zeros([1,47])
            mUser1[0,:] = np.concatenate((feat,mfeat,ifourier,mfourier,feat1,feat2,i_feat,m_feat,i_fourier,m_fourier), axis=None)
            #---------------------------------------------------------------------------------------------------------------------------
            #CLASIFICACION
            #CARGAR DATOS DE ENTRENAMIENTO
